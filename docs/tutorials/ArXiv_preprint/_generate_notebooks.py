...
# ═══════════════════════════════════════════════════════════════════════════
#  02  Kinetic equilibration of pure water with the atmosphere
# ═══════════════════════════════════════════════════════════════════════════
...
result = Simulation(cvs={"main": cv}, label="usecase02", solver=bdf_solver).run(
...
for i, kLa_i in enumerate(kLa_sweep):
    ...
    res_i = Simulation(cvs={"main": cv_i}, label=f"usecase02_sweep_{i}", solver=bdf_solver).run(
...
def _run_one_experimental_hour(kLa_i):
    ...
    Simulation(cvs={"main": cv_i}, label="usecase02_timing", solver=bdf_solver).run(
...
save_path_kinetic = HERE / "02_kinetic_co2_equilibration_microplate_well.ipynb"
...
# ═══════════════════════════════════════════════════════════════════════════
#  03  CSTR dilution-rate sweep — the E. coli/acetic-acid chemostat
# ═══════════════════════════════════════════════════════════════════════════
...
with plt.rc_context(PUB_STYLE):
    ...
    for ext in ("pdf", "png"):
        fig.savefig(FIG_DIR / f"03_cstr_dilution_rate_sweep.{ext}",
                    dpi=PUB_DPI, bbox_inches="tight")
    plt.show()

print(f"Saved publication figure to {FIG_DIR / '03_cstr_dilution_rate_sweep.png'}")
"""),
...
save_path_cstr = HERE / "03_cstr_dilution_rate_sweep.ipynb"
