...
(`reactions/equilibrium/reaction.py:126,136`) or
`KineticReaction(stoichiometry="...", ...)` (`reactions/kinetic/reaction.py:90,96`)
...
### 2. `reactions/equilibrium/interphase.py` — partition-model species fields

`_resolve_species()` (`interphase.py:53-70`) takes
`Union[str, Species, None]`. If given a `Species` object it returns it
unchanged; if given a string, it builds the *same* kind of
`vars(common_species).values()` catalog inline (`interphase.py:62-64`,
...
(`interphase.py:113-114`) and `RaoultEquilibrium.gas_species`/
`.liquid_species` (`interphase.py:278-279`) are typed
`Union[str, Species, None]` and resolved through this function
(`interphase.py:213-214, 348-349`). `RaoultEquilibrium.liquid_species`
and `.gas_species` both *default* to the literal string `"H2O"`
(`interphase.py:278-279`) — meaning a `RaoultEquilibrium()` constructed
...
## Phase 2: `reactions/equilibrium/interphase.py` (hardest — needs an API decision)
