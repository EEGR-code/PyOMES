...
- **Changing the operator-split ordering** (feed → reactions →
  transfer). That's `docs/phases-shipped/ORDERING.md` territory.
...
- [x] [`docs/phases-upcoming/STATE_UNIFICATION.md`](STATE_UNIFICATION.md):
      no live references (only `[[integrator-removal-phase-designed]]`
      memory link, which is intentional and stays).
- [x] [`docs/phases-upcoming/SIMULATION_CLASS.md`](SIMULATION_CLASS.md):
...
this checklist + design doc + phases-upcoming/README.md "Currently
in flight" entry (all of which will move to phases-shipped/ at
checkpoint 8).
...
- [ ] Move [`INTEGRATOR_REMOVAL.md`](INTEGRATOR_REMOVAL.md) and this
      checklist to
      [`../phases-shipped/`](../phases-shipped/) with "Shipped"
      banners at the top of each, mirroring the chemistry-unification
      precedent.
- [ ] [`docs/phases-upcoming/README.md`](README.md): remove
