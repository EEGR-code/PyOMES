...
- [ ] **Search** the repo for stale references to `run_batch`,
      ...
      [`docs/phases-shipped/`](../phases-shipped/).
- [ ] Delete
...
returns empty. Test suite passes. Documentation in `docs/phases-shipped/` still mentions these as
...
- [ ] Move
      [`docs/phases-upcoming/SIMULATION_CLASS.md`](SIMULATION_CLASS.md)
      and this checklist to
      [`docs/phases-shipped/`](../phases-shipped/). Add a status
...
- [ ] Update
      [`docs/phases-upcoming/README.md`](README.md):
...
- [ ] Update
      [`docs/phases-shipped/README.md`](../phases-shipped/README.md)
      with the new entry.

Sanity check: Architecture doc no longer references `run_batch` or `MultiCVSystem` in current-state
descriptions. Class diagram reflects the new shape. The phase docs sit in `phases-shipped/` with a
clear status banner.
...
- [ ] Update [`docs/phases-upcoming/README.md`](README.md)
