...
`BisectionChemicalEquilibriumEngine` solves aqueous acid-base speciation from a set of declared equilibrium reactions, rather than a fixed tier of hardcoded chemistry. Activity correction is optional: ideal by default, or a Davies/SIT ionic-strength correction, selected with one `activity_model` argument (`"ideal"`, `"davies"`, `"sit"`, or a model object such as `SITLiquidModel(epsilon=...)`), for example `StirredTankBuilder().chemistry(activity_model="davies")`.
