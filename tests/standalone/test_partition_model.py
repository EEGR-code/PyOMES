# -*- coding: utf-8 -*-
"""Tests for PartitionModel protocol and HenryEquilibrium (C2)."""
...
# A representative H2S HenryEquilibrium: kH ≈ 0.10 mol/(L·atm) at 298.15 K.
...
class TestHenryPartitionPartitionRatio:
    ...
    def test_partition_ratio_returns_float_not_none(self):
        # HenryEquilibrium is a linear model — partition_ratio() must return a float
        hp = self._hp()
...
class TestHenryPartitionActivityCorrection:
    ...
    def test_ideal_thermo_no_change(self):
        ...
        r_base  = HenryEquilibrium(H_ref=_H2S_H_REF, dlnH=0.0).partition_ratio(1.0, 1.0, _T_REF)
    ...
    def test_activity_correction_equilibrium_moles(self):
        ...
        n_liq = hp.equilibrium_a_moles(1.0, 1.0, 1.0, _T_REF,
                                        species_id="HS-", x_mol=x, charge=ch)
        n_liq_bare = HenryEquilibrium(H_ref=_H2S_H_REF, dlnH=0.0).equilibrium_a_moles(
            1.0, 1.0, 1.0, _T_REF)
    ...
    def test_thermo_field_excluded_from_equality(self):
        ...
        hp2 = HenryEquilibrium(H_ref=_H2S_H_REF, dlnH=0.0)
        assert hp1 == hp2
