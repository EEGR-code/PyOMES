...
| [`reactions/`](reactions/) | `ReactionBuilder` constructs stoichiometrically validated `KineticReaction` / `EquilibriumReaction` objects from an organism formula and balance mode; elemental-balance errors (`StoichiometryError`) raise at construction. `HenryEquilibrium`, `RaoultEquilibrium` and `KspEquilibrium` are gas-liquid and solid-liquid equilibrium constraints that double as `PartitionModel` instances. `ReactionSystem` aggregates reactions for a `ControlVolume`; `BlackBoxReactionModel` wraps external kinetic functions behind the `ReactionModel` protocol. Growth rate laws (`Monod`, `Contois`, `Andrews`, and five others) live in `rate_laws.py`. |
